...
/**
 * 
 * @author Alexander01998, AKA: the wurst dev
 *
 */
public class MiscUtils {
	...
	public static boolean openLink(final String url) {
		try {
			...
			MiscUtils.logger.error("Failed to open link", e);
			return false;
		}
	}
	...
	public static void openFile(final String path) {
		...
		try {
			...
			MiscUtils.logger.error("Failed to open file via Desktop.", e);
		}
		Label_0136: {
			if (Util.getOSType() == Util.EnumOS.WINDOWS) {
				...
				try {
					...
					MiscUtils.logger.error("Failed to open file", var8);
					break Label_0136;
				}
			}
			if (Util.getOSType() == Util.EnumOS.OSX) {
				try {
					...
					MiscUtils.logger.error("Failed to open file", var9);
				}
			}
		}
	...
	public static String post(final URL url, final String content, final String contentType) throws IOException {
		...
		connection.setRequestProperty("Content-Length",
				new StringBuilder().append(content.getBytes().length).toString());
