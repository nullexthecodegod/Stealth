...
public class ItemRenderer {
	...
	private void func_178109_a(AbstractClientPlayer clientPlayer) {
		int var2 = this.mc.theWorld.getCombinedLight(
				new BlockPos(clientPlayer.posX, clientPlayer.posY + clientPlayer.getEyeHeight(), clientPlayer.posZ), 0);
	...
	private void func_178110_a(EntityPlayerSP entityplayerspIn, float partialTicks) {
		float var3 = entityplayerspIn.prevRenderArmPitch
				+ (entityplayerspIn.renderArmPitch - entityplayerspIn.prevRenderArmPitch) * partialTicks;
		float var4 = entityplayerspIn.prevRenderArmYaw
	...
	private void func_178098_a(float p_178098_1_, AbstractClientPlayer clientPlayer) {
		...
		float var3 = this.itemToRender.getMaxItemUseDuration()
				- (clientPlayer.getItemInUseCount() - p_178098_1_ + 1.0F);
		...
		if (this.mc.thePlayer.isEntityInsideOpaqueBlock()) {
			...
			if (var2.getBlock().getRenderType() != -1) {
				this.func_178108_a(partialTicks,
						this.mc.getBlockRendererDispatcher().getBlockModelShapes().getTexture(var2));
			}
		}
