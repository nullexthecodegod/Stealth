...
public abstract class Render {
	...
	public boolean shouldRender(Entity livingEntity, ICamera camera, double camX, double camY, double camZ) {
		return livingEntity.isInRangeToRender3d(camX, camY, camZ) && (livingEntity.ignoreFrustumCheck
				|| camera.isBoundingBoxInFrustum(livingEntity.getEntityBoundingBox()));
	}
	...
	protected void renderOffsetLivingLabel(Entity entityIn, double x, double y, double z, String str, float p_177069_9_,
			double p_177069_10_) {
	...
	private void renderShadow(Entity entityIn, double x, double y, double z, float shadowAlpha, float partialTicks) {
		if (!Config.isShaders() || !Shaders.shouldSkipDefaultShadow) {
			...
			double var351 = entityIn.lastTickPosX + (entityIn.posX - entityIn.lastTickPosX) * partialTicks;
	...
	private void func_180549_a(Block blockIn, double p_180549_2_, double p_180549_4_, double p_180549_6_, BlockPos pos,
		...
		if (blockIn.isFullCube()) {
			...
			double var19 = (p_180549_9_ - (p_180549_4_ - (pos.getY() + p_180549_13_)) / 2.0D) * 0.5D
					* this.getWorldFromRenderManager().getLightBrightness(pos);
	...
	protected void renderLivingLabel(Entity entityIn, String str, double x, double y, double z, int maxDistance) {
		...
		EventNametag event = new EventNametag();
		event.fire();
		if (event.isCanceled())
			return;
