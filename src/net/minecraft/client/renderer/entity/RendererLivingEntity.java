...
import nl.x.client.cheat.cheats.display.ESP;
...
public abstract class RendererLivingEntity extends Render {
	...
	public void doRender(EntityLivingBase entity, double x, double y, double z, float entityYaw, float partialTicks) {
		if (!Reflector.RenderLivingEvent_Pre_Constructor.exists()
			...
			if (Config.isShaders() && Shaders.useEntityHurtFlash) {
				Shaders.setEntityHurtFlash(entity.hurtTime <= 0 && entity.deathTime <= 0 ? 0 : 102,
						this.getColorMultiplier(entity, entity.getBrightness(partialTicks), partialTicks));
			}
			...
			try {
				float var19 = this.interpolateRotation(entity.prevRenderYawOffset, entity.renderYawOffset,
						partialTicks);
				float var11 = this.interpolateRotation(entity.prevRotationYawHead, entity.rotationYawHead,
						partialTicks);
				...
				float var201 = entity.prevRotationPitch
				...
				float var16 = entity.prevLimbSwingAmount
	 ...
	 */
	protected void renderModel(EntityLivingBase entitylivingbaseIn, float p_77036_2_, float p_77036_3_,
		...
		if (var8 || var9) {
			...
			if (entitylivingbaseIn instanceof EntityPlayer && CheatManager.getCheat(ESP.class).isEnabled()
					&& ESP.mode.getValue().toString().equalsIgnoreCase("outline")) {
				GlStateManager.pushMatrix();
				...
				GlStateManager.disableTexture2D();
				RenderUtil.INSTANCE.setColor(Color.RED.getRGB());
				this.mainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_,
						p_77036_7_);
				GL11.glPopAttrib();
	...
	protected void rotateCorpse(EntityLivingBase bat, float p_77043_2_, float p_77043_3_, float partialTicks) {
		...
		if (bat.deathTime > 0) {
			float var6 = (bat.deathTime + partialTicks - 1.0F) / 20.0F * 1.6F;
	...
	protected void renderLayers(EntityLivingBase entitylivingbaseIn, float p_177093_2_, float p_177093_3_,
		...
		while (var9.hasNext()) {
			...
			var10.doRenderLayer(entitylivingbaseIn, p_177093_2_, p_177093_3_, partialTicks, p_177093_5_, p_177093_6_,
	...
	public void passSpecialRender(EntityLivingBase entitylivingbaseIn, double x, double y, double z) {
		if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(
				...
				new Object[] { entitylivingbaseIn, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z) })) {
				...
				if (var8 < var10 * var10) {
					...
					if (entitylivingbaseIn.isSneaking()) {
						...
						GlStateManager.translate((float) x,
								(float) y + entitylivingbaseIn.height + 0.5F
										- (entitylivingbaseIn.isChild() ? entitylivingbaseIn.height / 2.0F : 0.0F),
								(float) z);
						...
						this.renderOffsetLivingLabel(entitylivingbaseIn, x,
								y - (entitylivingbaseIn.isChild() ? (double) (entitylivingbaseIn.height / 2.0F) : 0.0D),
								z, var11, 0.02666667F, var8);
					}
				}
			}

			if (!Reflector.RenderLivingEvent_Specials_Post_Constructor.exists() || !Reflector
					.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Post_Constructor, new Object[] {
							entitylivingbaseIn, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z) })) {
				;
			}
		}
	}
