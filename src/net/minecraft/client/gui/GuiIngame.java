...
public class GuiIngame extends Gui {
		...
		if (this.field_175195_w > 0) {
			...
			if (var8 > 8) {
				...
				this.getFontRenderer().drawString(this.field_175201_x,
						-this.getFontRenderer().getStringWidth(this.field_175201_x) / 2, -10.0F, 16777215 | var9, true);
				GlStateManager.popMatrix();
				GlStateManager.pushMatrix();
				GlStateManager.scale(2.0F, 2.0F, 2.0F);
				this.getFontRenderer().drawString(this.field_175200_y,
						-this.getFontRenderer().getStringWidth(this.field_175200_y) / 2, 5.0F, 16777215 | var9, true);
				GlStateManager.popMatrix();
				GlStateManager.disableBlend();
				GlStateManager.popMatrix();
			}
	...
	protected void renderTooltip(ScaledResolution sr, float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			this.drawTexturedModalRect(var4 - 91 - 1 + var3.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1,
					0, 22, 24, 22);
			this.zLevel = var5;
	...
	private void renderPlayerStats(ScaledResolution p_180477_1_) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			boolean var4 = this.healthUpdateCounter > this.updateCounter
					&& (this.healthUpdateCounter - this.updateCounter) / 3L % 2L == 1L;
	...
	private void renderBossHealth() {
		if (BossStatus.bossName != null && BossStatus.statusBarTime > 0) {
			...
			this.getFontRenderer().drawStringWithShadow(var8,
					var3 / 2 - this.getFontRenderer().getStringWidth(var8) / 2, var7 - 10, bossTextColor);
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
			this.mc.getTextureManager().bindTexture(icons);
		}
	}
	...
	private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
		if (Config.isVignetteEnabled()) {
			...
			double var5 = Math.min(var3.getResizeSpeed() * var3.getWarningTime() * 1000.0D,
