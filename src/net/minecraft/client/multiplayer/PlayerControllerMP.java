...
public class PlayerControllerMP {
	 ...
	 * @param side
	 *            The side it was destroyed from
	 */
	 ...
	 * 
	 * @param loc
	 *            location of the block being clicked
	 * @param face
	 *            Blockface being clicked
	 */
				...
				if (this.stepSoundTickCounter % 4.0F == 0.0F) {
					this.mc.getSoundHandler()
							.playSound(new PositionedSoundRecord(new ResourceLocation(var3.stepSound.getStepSound()),
									...
									posBlock.getX() + 0.5F, posBlock.getY() + 0.5F, posBlock.getZ() + 0.5F));
				}
		...
		if (!this.mc.theWorld.getWorldBorder().contains(hitPos)) {
			...
			if (this.currentGameType != WorldSettings.GameType.SPECTATOR) {
				...
				if ((!player.isSneaking() || player.getHeldItem() == null)
						&& var11.getBlock().onBlockActivated(worldIn, hitPos, var11, player, side, var7, var8, var9)) {
					var10 = true;
				}
			...
			this.netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(hitPos, side.getIndex(),
					player.inventory.getCurrentItem(), var7, var8, var9));
	...
	public ItemStack windowClick(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn) {
		...
		this.netClientHandler
				.addToSendQueue(new C0EPacketClickWindow(windowId, slotId, mouseButtonClicked, mode, var7, var6));
		return var7;
	}
