...
import nl.x.api.event.impl.EventChat;
...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void moveEntity(double x, double y, double z) {
		EventMove e = new EventMove(x, z, y);
		e.fire();
		if (e.isCanceled())
			return;
	...
	public void onUpdateWalkingPlayer() {
		...
		EventUpdate event = new EventUpdate(this.rotationYaw, this.rotationPitch, this.posY, this.onGround, State.pre);
		event.fire();
		if (event.isCanceled()) {
			new EventUpdate(this.rotationYaw, this.rotationPitch, this.posY, this.onGround, State.post).fire();
			return;
		}
		...
		if (this.isCurrentViewEntity()) {
			...
			double var5 = event.getY() - this.lastReportedPosY;
			double var7 = this.posZ - this.lastReportedPosZ;
			double var9 = event.getYaw() - this.lastReportedYaw;
			double var11 = event.getPitch() - this.lastReportedPitch;
			...
			for (int y = MathHelper.floor_double(this.boundingBox.minY); y < MathHelper
				...
				for (int z = MathHelper.floor_double(this.boundingBox.minZ); z < MathHelper
					...
					if ((block != null) && (!(block instanceof BlockAir))) {
						AxisAlignedBB boundingBox = block.getCollisionBoundingBox(this.mc.theWorld,
								new BlockPos(x, y, z), this.mc.theWorld
	...
	public boolean isOnLiquid() {
		...
		int y = (int) this.boundingBox.offset(0.0D, -0.01D, 0.0D).minY;
		for (int x = MathHelper.floor_double(this.boundingBox.minX); x < MathHelper.floor_double(this.boundingBox.maxX)
				+ 1; x++) {
			for (int z = MathHelper.floor_double(this.boundingBox.minZ); z < MathHelper
					.floor_double(this.boundingBox.maxZ) + 1; z++) {
	...
	public void onLivingUpdate() {
		...
		if (this.isUsingItem() && !this.isRiding()) {
			EventSlowdown event = new EventSlowdown(From.item, 0.2);
			event.fire();
			if (!event.isCanceled()) {
				this.movementInput.moveStrafe *= event.getModifier();
				this.movementInput.moveForward *= event.getModifier();
	...
	public boolean isMoving() {
		return (Math.abs(mc.thePlayer.moveForward) + Math.abs(mc.thePlayer.moveStrafing) != 0.0)
				|| (mc.thePlayer.moveForward != 0.0 || mc.thePlayer.moveStrafing != 0.0);
	}
	...
	public void setSpeed(Number speed) {
		MoveUtils.setSpeed(speed.doubleValue());
	}
	...
	public void setModifiyedSpeed(Number speed) {
		this.motionX = (-(Math.sin(getDirection()) * speed.floatValue()));
		this.motionZ = (Math.cos(getDirection()) * speed.floatValue());
	}
	...
	public boolean isEndOfLadder() {
		...
		final int j = MathHelper.floor_double(this.getEntityBoundingBox().maxY);
