...
public abstract class World implements IBlockAccess {
	 ...
	 *            block from being re-rendered, if this is a client world. Flags
	...
	public BlockPos getHeight(BlockPos pos) {
		...
		if (pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000) {
			if (this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, true)) {
				var2 = this.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4).getHeightValue(pos.getX() & 15,
						pos.getZ() & 15);
			} else {
				var2 = 0;
			}
	...
	public MovingObjectPosition rayTraceBlocks(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid,
		...
		if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
			if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
				...
				if ((!ignoreBlockWithoutBoundingBox || var15.getCollisionBoundingBox(this, var12, var14) != null)
						&& var15.canCollideCheck(var14, stopOnLiquid)) {
				...
				while (var42-- >= 0) {
					...
					if (!ignoreBlockWithoutBoundingBox || var39.getCollisionBoundingBox(this, var12, var38) != null) {
						if (var39.canCollideCheck(var38, stopOnLiquid)) {
							...
							var41 = new MovingObjectPosition(MovingObjectPosition.MovingObjectType.MISS, vec31, var37,
									var12);
						}
					}
				}
	...
	public void playSoundAtEntity(Entity entityIn, String name, float volume, float pitch) {
		for (int var5 = 0; var5 < this.worldAccesses.size(); ++var5) {
			((IWorldAccess) this.worldAccesses.get(var5)).playSound(name, entityIn.posX, entityIn.posY, entityIn.posZ,
					volume, pitch);
		}
	}
	...
	public void playSoundToNearExcept(EntityPlayer player, String name, float volume, float pitch) {
		for (int var5 = 0; var5 < this.worldAccesses.size(); ++var5) {
			((IWorldAccess) this.worldAccesses.get(var5)).playSoundToNearExcept(player, name, player.posX, player.posY,
	...
	private void spawnParticle(int particleID, boolean p_175720_2_, double xCood, double yCoord, double zCoord,
		...
		for (int var16 = 0; var16 < this.worldAccesses.size(); ++var16) {
			((IWorldAccess) this.worldAccesses.get(var16)).spawnParticle(particleID, p_175720_2_, xCood, yCoord, zCoord,
	 ...
	 * 
	 * @param pos
	 *            The object containing the x and z coordinates to check at
	 */
	public BlockPos getTopSolidOrLiquidBlock(BlockPos pos) {
		...
		for (var3 = new BlockPos(pos.getX(), var2.getTopFilledSegment() + 16, pos.getZ()); var3
				.getY() >= 0; var3 = var4) {
			var4 = var3.down();
	...
	public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
		...
		if (!forceUpdate || this.isAreaLoaded(var3 - var5, 0, var4 - var5, var3 + var5, 0, var4 + var5, true)) {
			...
			if (!entityIn.addedToChunk || entityIn.chunkCoordX != var6 || entityIn.chunkCoordY != var7
					|| entityIn.chunkCoordZ != var8) {
	...
	public boolean checkNoEntityCollision(AxisAlignedBB bb, Entity entityIn) {
		...
		for (int var4 = 0; var4 < var3.size(); ++var4) {
			...
			if (!var5.isDead && var5.preventEntitySpawning && var5 != entityIn
					&& (entityIn == null || entityIn.ridingEntity != var5 && entityIn.riddenByEntity != var5)) {
				return false;
			}
		}
				...
				for (int var13 = var6; var13 < var7; ++var13) {
					for (int var14 = var8; var14 < var9; ++var14) {
						...
						if (var17.getMaterial() == materialIn) {
							double var18 = var13 + 1 - BlockLiquid
	 ...
	 * 
	 * @param player
	 *            The player putting out the fire
	 * @param pos
	 *            The coordinates of the fire
	 * @param side
	 *            The side from which the player is putting out the fire from
	 */
	...
	public static boolean doesBlockHaveSolidTopSurface(IBlockAccess blockAccess, BlockPos pos) {
		...
		return var3.getMaterial().isOpaque() && var3.isFullCube() ? true
				: (var3 instanceof BlockStairs ? var2.getValue(BlockStairs.HALF) == BlockStairs.EnumHalf.TOP
						: (var3 instanceof BlockSlab ? var2.getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.TOP
								: (var3 instanceof BlockHopper ? true
										: (var3 instanceof BlockSnow
												? ((Integer) var2.getValue(BlockSnow.LAYERS)).intValue() == 7
												: false))));
	}
		...
		if (this.ambientTickCountdown == 0 && !this.isRemote) {
			...
			if (var9.getMaterial() == Material.air && this.getLight(var8) <= this.rand.nextInt(8)
				...
				if (var10 != null && var10.getDistanceSq(var5 + 0.5D, var7 + 0.5D, var6 + 0.5D) > 4.0D) {
					this.playSoundEffect(var5 + 0.5D, var7 + 0.5D, var6 + 0.5D, "ambient.cave.cave", 0.7F,
	 ...
	 * @param noWaterAdj
	 *            If true, this method will only return true if there is a
		...
		if (var4 > 0.15F) {
			...
			if (pos.getY() >= 0 && pos.getY() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10) {
				...
				if ((var6 == Blocks.water || var6 == Blocks.flowing_water)
					...
					boolean var7 = this.isWater(pos.west()) && this.isWater(pos.east()) && this.isWater(pos.north())
	 ...
	 *            The block coordinates
	 * @param checkLight
	 *            If false, checking for the correct light values will be
	 *            skipped
	 */
		...
		for (int var9 = var5; var9 <= var6; ++var9) {
			for (int var10 = var7; var10 <= var8; ++var10) {
				if (this.isChunkLoaded(var9, var10, true)) {
					this.getChunkFromChunkCoords(var9, var10).getEntitiesWithinAABBForEntity(entityIn, boundingBox,
							var4, predicate);
				}
			}
		}
	...
	public boolean canBlockBePlaced(Block blockIn, BlockPos pos, boolean p_175716_3_, EnumFacing side, Entity entityIn,
		...
		return var8 != null && !this.checkNoEntityCollision(var8, entityIn) ? false
				: (var7.getMaterial() == Material.circuits && blockIn == Blocks.anvil ? true
						: var7.getMaterial().isReplaceable() && blockIn.canReplace(this, pos, side, itemStackIn));
	}
	...
	public boolean canLightningStrike(BlockPos strikePosition) {
		if (!this.isRaining()) {
			...
			return var2.getEnableSnow() ? false
					: (this.canSnowAt(strikePosition, false) ? false : var2.canSpawnLightningBolt());
		}
	}
	...
	public Random setRandomSeed(int p_72843_1_, int p_72843_2_, int p_72843_3_) {
		long var4 = p_72843_1_ * 341873128712L + p_72843_2_ * 132897987541L + this.getWorldInfo().getSeed()
				+ p_72843_3_;
