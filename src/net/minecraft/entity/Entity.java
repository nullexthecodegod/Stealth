...
import nl.x.api.event.impl.EventSafewalk;
...
public abstract class Entity implements ICommandSender {
	...
	protected void setSize(float width, float height) {
		if (width != this.width || height != this.height) {
			...
			this.setEntityBoundingBox(new AxisAlignedBB(this.getEntityBoundingBox().minX,
					this.getEntityBoundingBox().minY, this.getEntityBoundingBox().minZ,
					this.getEntityBoundingBox().minX + this.width, this.getEntityBoundingBox().minY + this.height,
	...
	public boolean onLand() {
		...
		for (int x = MathHelper.floor_double(this.posX); x < MathHelper.ceiling_double_int(this.posX); ++x) {
			for (int z = MathHelper.floor_double(this.posZ); z < MathHelper.ceiling_double_int(this.posZ); ++z) {
				...
				if (Minecraft.getMinecraft().theWorld.getBlockState(pos).getBlock().isCollidable()) {
					return true;
				}
			}
		}
	...
	public void moveEntity(double x, double y, double z) {
		if (this.noClip) {
			...
			double var15 = y;
			double var17 = z;
			EventSafewalk pre = new EventSafewalk(State.pre);
			EventSafewalk safe = new EventSafewalk(State.post);
			pre.fire();
			boolean var19 = (this.onGround && this.isSneaking() && this instanceof EntityPlayer) || (pre.isCanceled());
			...
			EventStep step = new EventStep(this.stepHeight, this, State.pre);
			step.fire();
			if (step.getStepHeight() > 0.0F && var54 && (var13 != x || var17 != z)) {
				double var56 = x;
				double var25 = y;
				double var27 = z;
				...
				if (var56 * var56 + var27 * var27 >= x * x + z * z) {
					...
					new EventStep(step.getStepHeight(), this, State.post).fire();
				}
			}
	...
	public boolean isWet() {
		return this.inWater || this.worldObj.canLightningStrike(new BlockPos(this.posX, this.posY, this.posZ))
				|| this.worldObj.canLightningStrike(new BlockPos(this.posX, this.posY + this.height, this.posZ));
	}
	...
	public MovingObjectPosition rayTrace(double blockReachDistance, float partialTicks) {
		...
		Vec3 var6 = var4.addVector(var5.xCoord * blockReachDistance, var5.yCoord * blockReachDistance,
